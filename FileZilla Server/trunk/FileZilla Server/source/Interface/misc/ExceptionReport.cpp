...
LONG WINAPI CExceptionReport::UnhandledExceptionFilter(PEXCEPTION_POINTERS pExceptionInfo)
{
	...
	if (::MessageBox(NULL,
_T("An unhandled exception has occurred in FileZilla Server Interface\r\n\
FileZilla Server Interface has to be closed.\r\n\r\n\
...
If you're using the latest version of FileZilla Server Interface, please send the generated exception record to the following mail address: Tim.Kosse@gmx.de\r\n\
The report will be analyzed and the reason for this exception will be fixed in the next version of FileZilla Server Interface.\r\n\r\n\
Please note: It may be possible - though unlikely - that the exception report may contain personal and or confidential information. All exception reports will be processed higly confidential and solely to analyze the crash. The reports will be deleted immediately after processing.\r\n"),
		_T("FileZilla Server Interface - Unhandled exception"), MB_APPLMODAL | MB_YESNO | MB_ICONSTOP)==IDYES)
	{
		...
		if (nError)
		{
			...
			MessageBox(0, tmp, _T("FileZilla Server Interface - Unhandled eception"), MB_APPLMODAL | MB_OK | MB_ICONEXCLAMATION);
		}
		else
		{
			sendMail();
...
Please make sure that you are using the latest version of FileZilla Server Interface.\n\
You can download the latest version from http://sourceforge.net/projects/filezilla/.\n\
If you do use the latest version, please send the exception report to Tim.Kosse@gmx.de along with a brief explanation what you did before FileZilla Server Interface crashed."), m_pLogFileName, m_pDmpFileName);
			MessageBox(0, tmp, _T("FileZilla Server Interface - Unhandled eception"), MB_APPLMODAL | MB_OK | MB_ICONEXCLAMATION);
...
int CExceptionReport::sendMail()
{
	...
	mail.SetMessage(_T("Enter your comments here, what did you do with FileZilla Server Interface before it crashed?"));

	mail.AddAttachment(m_pLogFileName, _T("FileZillaServerInterface.rpt"));
	mail.AddAttachment(m_pDmpFileName, _T("FileZillaServerInterface.dmp"));
