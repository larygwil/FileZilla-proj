...
void init()
{
	...
	// Tweaked values
	material_values[pieces::pawn]   = score( 70, 79 );
	material_values[pieces::knight] = score( 366, 362 );
	material_values[pieces::bishop] = score( 418, 377 );
	material_values[pieces::rook]   = score( 477, 631 );
	material_values[pieces::queen]  = score( 1110, 1137 );
	double_bishop                   = score( 41, 35 );
	passed_pawn_advance_power       = score( 210, 169 );
	passed_pawn_king_distance[0]    = score( 0, 5 );
	passed_pawn_king_distance[1]    = score( 0, 3 );
	passed_pawn_base[0]             = score( 3, 9 );
	passed_pawn_base[1]             = score( 0, 1 );
	passed_pawn_base[2]             = score( 0, 0 );
	passed_pawn_base[3]             = score( 1, 0 );
	doubled_pawn_base[0][0]         = score( 17, 5 );
	doubled_pawn_base[0][1]         = score( 5, 1 );
	doubled_pawn_base[0][2]         = score( 2, 0 );
	doubled_pawn_base[0][3]         = score( 2, 0 );
	doubled_pawn_base[1][0]         = score( 12, 1 );
	doubled_pawn_base[1][1]         = score( 0, 0 );
	doubled_pawn_base[1][2]         = score( 1, 0 );
	doubled_pawn_base[1][3]         = score( 0, 0 );
	backward_pawn_base[0][0]        = score( 8, 4 );
	backward_pawn_base[0][1]        = score( 2, 0 );
	backward_pawn_base[0][2]        = score( 0, 0 );
	backward_pawn_base[0][3]        = score( 2, 0 );
	backward_pawn_base[1][0]        = score( 9, 7 );
	backward_pawn_base[1][1]        = score( 1, 2 );
	backward_pawn_base[1][2]        = score( 2, 1 );
	backward_pawn_base[1][3]        = score( 0, 0 );
	isolated_pawn_base[0][0]        = score( 12, 8 );
	isolated_pawn_base[0][1]        = score( 4, 2 );
	isolated_pawn_base[0][2]        = score( 1, 1 );
	isolated_pawn_base[0][3]        = score( 4, 0 );
	isolated_pawn_base[1][0]        = score( 6, 19 );
	isolated_pawn_base[1][1]        = score( 2, 0 );
	isolated_pawn_base[1][2]        = score( 1, 0 );
	isolated_pawn_base[1][3]        = score( 2, 0 );
	connected_pawn_base[0][0]       = score( 0, 4 );
	connected_pawn_base[0][1]       = score( 0, 0 );
	connected_pawn_base[0][2]       = score( 0, 1 );
	connected_pawn_base[0][3]       = score( 0, 0 );
	connected_pawn_base[1][0]       = score( 4, 3 );
	connected_pawn_base[1][1]       = score( 0, 0 );
	connected_pawn_base[1][2]       = score( 0, 0 );
	connected_pawn_base[1][3]       = score( 0, 1 );
	candidate_passed_pawn_base[0]   = score( 9, 21 );
	candidate_passed_pawn_base[1]   = score( 0, 0 );
	candidate_passed_pawn_base[2]   = score( 3, 6 );
	candidate_passed_pawn_base[3]   = score( 3, 7 );
	pawn_shield[0]                  = score( 36, 0 );
	pawn_shield[1]                  = score( 22, 0 );
	pawn_shield[2]                  = score( 15, 0 );
	pawn_shield_attack[0]           = score( 3, 0 );
	pawn_shield_attack[1]           = score( 2, 0 );
	pawn_shield_attack[2]           = score( 2, 0 );
	absolute_pin[1]                 = score( 0, 3 );
	absolute_pin[2]                 = score( 0, 28 );
	absolute_pin[3]                 = score( 0, 0 );
	absolute_pin[4]                 = score( 0, 0 );
	absolute_pin[5]                 = score( 0, 2 );
	rooks_on_open_file              = score( 27, 8 );
	rooks_on_half_open_file         = score( 11, 2 );
	...
	king_attack_by_piece[4]         = 16;
	king_attack_by_piece[5]         = 18;
	king_check_by_piece[2]          = 1;
	king_check_by_piece[3]          = 6;
	king_check_by_piece[4]          = 4;
	king_check_by_piece[5]          = 13;
	king_melee_attack_by_rook       = 0;
	king_melee_attack_by_queen      = 41;
	king_attack_min[0]              = 7;
	king_attack_min[1]              = 0;
	king_attack_max[0]              = 230;
	king_attack_max[1]              = 99;
	king_attack_rise[0]             = 1;
	king_attack_rise[1]             = 1;
	king_attack_exponent[0]         = 108;
	king_attack_exponent[1]         = 100;
	king_attack_offset[0]           = 15;
	king_attack_offset[1]           = 100;
	king_attack_pawn_shield         = 71;
	center_control                  = score( 5, 0 );
	rule_of_the_square              = score( 0, 6 );
	passed_pawn_unhindered          = score( 3, 6 );
	defended_by_pawn[1]             = score( 3, 6 );
	defended_by_pawn[2]             = score( 4, 0 );
	defended_by_pawn[3]             = score( 4, 0 );
	defended_by_pawn[4]             = score( 0, 2 );
	defended_by_pawn[5]             = score( 0, 4 );
	attacked_piece[1]               = score( 2, 9 );
	attacked_piece[2]               = score( 1, 15 );
	attacked_piece[3]               = score( 10, 10 );
	attacked_piece[4]               = score( 5, 10 );
	attacked_piece[5]               = score( 13, 22 );
	hanging_piece[1]                = score( 3, 11 );
	hanging_piece[2]                = score( 5, 0 );
	hanging_piece[3]                = score( 5, 21 );
	hanging_piece[4]                = score( 11, 14 );
	hanging_piece[5]                = score( 14, 22 );
	mobility_knight_min             = score( -27, -46 );
	mobility_knight_max             = score( 0, 1 );
	mobility_knight_rise            = score( 4, 19 );
	mobility_knight_offset          = score( 1, 0 );
	mobility_bishop_min             = score( -73, -16 );
	mobility_bishop_max             = score( 0, 26 );
	mobility_bishop_rise            = score( 6, 5 );
	mobility_bishop_offset          = score( 0, 0 );
	mobility_rook_min               = score( -8, -57 );
	mobility_rook_max               = score( 0, 9 );
	mobility_rook_rise              = score( 22, 7 );
	mobility_rook_offset            = score( 7, 2 );
	mobility_queen_min              = score( -27, -51 );
	mobility_queen_max              = score( 0, 0 );
	mobility_queen_rise             = score( 1, 50 );
	mobility_queen_offset           = score( 0, 0 );
	side_to_move                    = score( 6, 0 );
	rooks_on_rank_7                 = score( 1, 50 );
	knight_outposts[0]              = score( 15, 5 );
	knight_outposts[1]              = score( 16, 21 );
	bishop_outposts[0]              = score( 4, 2 );
	bishop_outposts[1]              = score( 9, 11 );
	trapped_rook[0].mg()            = -24;
	trapped_rook[1].mg()            = -63;
	trapped_bishop                  = score( -17, -1 );
